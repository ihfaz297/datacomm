...
if ok:
    ...
    ax[0].set_title(f"first 200 samples of the WAV (Fs = {Fs} Hz)")
    ax[1].stem(np.arange(50) / Fs_ds, x_ds[:50], basefmt=" ")
    ax[1].set_title(f"downsampled by {M}: Fs = {Fs_ds} Hz")
    ax[2].plot(t[:200], x[:200], alpha=.4, label="original")
    ax[2].step(t[:200], x_q[:200], where="mid", label=f"{L} levels")
    ax[2].legend()
    ax[2].set_title("quantized")
